...
pub mod matwrapper;
pub use matwrapper::MatWrapper;

#[path = "lib/vecwrapper.rs"]
pub mod vecwrapper;
pub use vecwrapper::VecWrapper;
...
pub fn dist(x: VecWrapper<f32>, y: &VecWrapper<f32>) -> f32 {
    let mut res = 0.0;
    let mut i = 0;
    while i < x.len() {
...
pub fn add_to_row(x: &mut MatWrapper<f32>, row: usize, y: &VecWrapper<f32>) {
    let mut i = 0;
...
#[ensures(x.rows() == old(x.rows()))]
#[ensures(x.cols() == old(x.cols()))]
pub fn normalize_row(x: &mut MatWrapper<f32>, row: usize, n: usize) {
    let mut i = 0;
...
pub fn init_centers(n: usize, k: usize) -> MatWrapper<f32> {
...
pub fn nearest(p: &VecWrapper<f32>, cs: &MatWrapper<f32>) -> usize {
    // let n = p.len();
    let k = cs.rows();
    let mut res = 0;
    let mut min = f32::MAX;
...
pub fn normalize_centers(cs: &mut MatWrapper<f32>, weights: &VecWrapper<usize>) {
    let k = cs.rows();
    let mut i = 0;
    while i < k {
...
#[ensures(result.cols() == n)]
pub fn kmeans_step(n: usize, cs: MatWrapper<f32>, ps: &MatWrapper<f32>) -> MatWrapper<f32> {
    let k = cs.rows();
