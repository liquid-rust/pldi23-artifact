...
impl FOwnedComponents {
    ...
    #[flux::trusted]
    #[flux::sig(fn(oc: FOwnedComponents)
       -> Option<HostPath{v: v.depth == oc.depth && v.is_relative == oc.is_relative && v.non_symlink == (oc.size == oc.ns_prefix) && v.non_symlink_prefixes == (oc.size - 1 <= oc.ns_prefix)}>)]
