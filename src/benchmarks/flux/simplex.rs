...
#[flux::sig(fn(m:usize, n:usize, &RMat<f32>[m, n], j:usize{0 < j && j < n}, i0:usize{0 < i0 && i0 < m}, r0:f32) -> usize{v:0 < v && v < m})]
...
pub fn init_ratio_i(m: usize, _n: usize, arr2: &RMat<f32>, j: usize) -> usize {
    ...
    panic!() // abort for negative coefficients
}

#[flux::sig(fn(m:usize{0 < m}, n:usize{0 < n}, &RMat<f32>[m, n], j: usize{0 < j && j < n}, i:usize{0 < i && i < m}) -> f32)]
...
#[flux::sig(fn(m:usize, n:usize, arr2:&mut RMat<f32>[m,n], i:usize{0 < i && i < m}, j:usize{0 < j && j < n}))]
fn row_op(m: usize, n: usize, arr2: &mut RMat<f32>, i: usize, j: usize) {
    // Rename `jj` to `j` to see an error!
    let c = *arr2.get(i, j);
    let mut jj = 1;
    while jj < n {
        *arr2.get_mut(i, jj) /= c;
...
#[flux::sig(fn (m:usize{1 < m}, n:usize{2 < n}, arr2:&mut RMat<f32>[m, n]))]
pub fn simplex(m: usize, n: usize, arr2: &mut RMat<f32>) {
    while is_neg(arr2, m, n) {
        if unb1(m, n, arr2) {
            panic!(); // unbound solution
        } else {
